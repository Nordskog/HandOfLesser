        ...
        SimpleGestureState getGesture(SimpleGestureType type, TrackedHand* hand)
        {
            switch (type)
            {
                ...
                case SimpleGestureType::OpenHandFacingFace:
                {
                    return
                    {
                        (hand->mAimState.status & XR_HAND_TRACKING_AIM_SYSTEM_GESTURE_BIT_FB) == XR_HAND_TRACKING_AIM_SYSTEM_GESTURE_BIT_FB ? 1.0f : 0.0f,
                        (hand->mAimState.status & XR_HAND_TRACKING_AIM_SYSTEM_GESTURE_BIT_FB) == XR_HAND_TRACKING_AIM_SYSTEM_GESTURE_BIT_FB,
                    };
                }
            }
        }
