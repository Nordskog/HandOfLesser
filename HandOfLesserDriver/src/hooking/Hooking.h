...
// Stolen from OpenVR-SpaceCalibrator
class IHook
{
public:
...
public:
	...
	bool CreateHookInObjectVTable(void *object, int vtableOffset, void* detourFunction)
	{
		...
		{
			DriverLog(
				"Failed to create hook for %s, error: %s", name.c_str(), MH_StatusToString(err));
			return false;
		}
		...
		{
			DriverLog(
				"Failed to enable hook for %s, error: %s", name.c_str(), MH_StatusToString(err));
