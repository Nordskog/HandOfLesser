...
// https://gitlab.freedesktop.org/monado/monado/-/blob/main/src/xrt/auxiliary/util/u_hand_simulation.c
...
//-----------------------------------------------------------------------------
// Purpose: Sets up a default open hand pose which can then be manipulated with curl and splay
// values. Much of this is very approximate.
//-----------------------------------------------------------------------------
static void InitHand(HandSimHand& out_hand)
{
	...
	// Metacarpal splays for each of the fingers. Do this to add some spread to the fingers'
	// metacarpal (the joints that connect to the wrist).
	...
	// Proximal splays for each of the fingers. Do this to add some spread to the finger's proximal
	// joints. These joints are also the ones that splay with the values provided.
...
//-----------------------------------------------------------------------------
// Purpose: All fingers (expect thumb) have basically the same properties in terms of curl and
// splay. You can splay only the proximal, and curl the proximal, intermediate and distal joints.
// This applies the curl and splay to these joints.
//-----------------------------------------------------------------------------
static void
ApplyGenericFingerTransform(const float curl, const float splay, HandSimFinger& out_finger)
{
	out_finger.metacarpal.swing.v[0]
		+= DEG_TO_RAD(curl * 5.f); // the metacarpal only curls a little
...
// We have to do convert the quaternion as we use vr::HmdQuaternion_t for our representation, but
// OpenVR wants vr::HmdQuaternionf_t
//-----------------------------------------------------------------------------
...
)
{
	...
	// so we'll use this helper to convert
...
//-----------------------------------------------------------------------------
// Purpose: This is just a little helper function to make our calls when trying to compute each bone
// a little simpler by specifying just a float for joint length, instead of the whole vector
//-----------------------------------------------------------------------------
static void ComputeBoneTransform(
	const vr::ETrackedControllerRole role,
	const vr::HmdQuaternion_t& orientation,
	const float joint_length,
	vr::VRBoneTransform_t& out_transform
)
{
...
//-----------------------------------------------------------------------------
// Purpose: Takes an orientation and length of a joint and converts it to a vr::VRBoneTransform_t,
// but with the orientation applied to the offset We have to do convert the quaternion as we use
// vr::HmdQuaternion_t for our representation, but OpenVR wants vr::HmdQuaternionf_t
//-----------------------------------------------------------------------------
static void ComputeBoneTransformMetacarpal(
	const vr::ETrackedControllerRole role,
	const vr::HmdQuaternion_t& orientation,
	const float joint_length,
	vr::VRBoneTransform_t& out_transform
)
{
	...
	/*
	The Skeletal Input API is designed to be used with common industry tools, such as Maya, to make
	it easier to move content from 3D editors into VR. The way that FBX handles conversion to a
	different coordinate system is to transform the root bone (wrist), then counter-transform the
	root's children to account for the root's change, but then leave the local coordinate systems of
	the remaining bones as-is This means that the metacarpals will be rotated 90 degrees from the
	wrist if trying to build a skeleton programmatically. So, we apply this extra rotation to the
	metacarpals to account for this.
	*/
...
//-----------------------------------------------------------------------------
// Purpose: Get the OpenVR bone index from a finger (index=0, middle=1...) and the bone position in
// the finger. Used in ComputeSkeletalTransforms
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
static void
ComputeSkeletalTransforms(const HandSimHand& hand, vr::VRBoneTransform_t* out_transforms)
{
	// Do the thumb separately as it's special and not like the other fingers
	ComputeBoneTransformMetacarpal(
		hand.role,
		HmdQuaternion_FromSwingTwist(hand.thumb.metacarpal.swing, hand.thumb.metacarpal.twist),
		finger_joint_lengths[0][0],
		out_transforms[eBone_Thumb0]
	);
	ComputeBoneTransform(
		hand.role,
		HmdQuaternion_FromSwingTwist(hand.thumb.proximal.swing, hand.thumb.metacarpal.twist),
		finger_joint_lengths[0][1],
		out_transforms[eBone_Thumb1]
	);
	ComputeBoneTransform(
		hand.role,
		HmdQuaternion_FromEulerAngles(hand.thumb.distal.rotation, 0.f, 0.f),
		finger_joint_lengths[0][2],
		out_transforms[eBone_Thumb2]
	);
	ComputeBoneTransform(
		hand.role, HmdQuaternion_Identity, finger_joint_lengths[0][3], out_transforms[eBone_Thumb3]
	);
	...
	for (int finger = 0; finger < 4; finger++)
	{
		ComputeBoneTransformMetacarpal(
			...
			HmdQuaternion_FromSwingTwist(
				hand.fingers[finger].metacarpal.swing, hand.fingers[finger].metacarpal.twist
			),
			finger_joint_lengths[finger + 1][0],
			out_transforms[CalculateBoneTransformPositionFromFinger(finger, 0)]
		);

		ComputeBoneTransform(
			hand.role,
			HmdQuaternion_FromSwingTwist(
				hand.fingers[finger].proximal.swing, hand.fingers[finger].proximal.twist
			),
			finger_joint_lengths[finger + 1][1],
			out_transforms[CalculateBoneTransformPositionFromFinger(finger, 1)]
		);

		ComputeBoneTransform(
			hand.role,
			HmdQuaternion_FromEulerAngles(hand.fingers[finger].intermediate.rotation, 0.f, 0.f),
			finger_joint_lengths[finger + 1][2],
			out_transforms[CalculateBoneTransformPositionFromFinger(finger, 2)]
		);

		ComputeBoneTransform(
			hand.role,
			HmdQuaternion_FromEulerAngles(hand.fingers[finger].distal.rotation, 0.f, 0.f),
			finger_joint_lengths[finger + 1][3],
			out_transforms[CalculateBoneTransformPositionFromFinger(finger, 3)]
		);

		ComputeBoneTransform(
			hand.role,
			HmdQuaternion_Identity,
			finger_joint_lengths[finger + 1][4],
			out_transforms[CalculateBoneTransformPositionFromFinger(finger, 4)]
		);
	}
}
