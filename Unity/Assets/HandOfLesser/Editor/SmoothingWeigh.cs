...
namespace HOL
{
    class SmoothingWeigh
    {
        ...
        public static int generateSmoothingWeightAnimation(HandSide side, FingerType finger, FingerBendType joint, AnimationClipPosition position, PropertyType property)
        {
        ...
        public static BlendTree generateFlipflopBlendtreeInner( BlendTree parent, HandSide side, FingerType finger, FingerBendType joint, PropertyType drivingProperty, bool invert)
        {
            ...
            AnimationClip negativeAnimation = AssetDatabase.LoadAssetAtPath<AnimationClip>(
                HOL.Resources.getAnimationOutputPath(HOL.Resources.getAnimationClipName(side, finger, joint, PropertyType.smoothing_weight, firstPosition)));
            AnimationClip positiveAnimation = AssetDatabase.LoadAssetAtPath<AnimationClip>(
                HOL.Resources.getAnimationOutputPath(HOL.Resources.getAnimationClipName(side, finger, joint, PropertyType.smoothing_weight, secondPosition)));
        ...
        public static int generateWeighBlendtree(BlendTree parent, List<ChildMotion> childTrees, HandSide side, FingerType finger, FingerBendType joint)
        {
            ...
            tree.AddChild(generateFlipflopBlendtreeInner(tree, side, finger, joint, PropertyType.input, false), 0);
            tree.AddChild(generateFlipflopBlendtreeInner(tree, side, finger, joint, PropertyType.smooth, true), 1);
            ...
            foreach (HandSide side in new HandSide().Values())
            {
                foreach (FingerType finger in new FingerType().Values())
                {
                    foreach (FingerBendType joint in new FingerBendType().Values())
                    {
                        ...
                        controller.AddParameter(HOL.Resources.getJointParameterName(side, finger, joint, PropertyType.smoothing_weight), AnimatorControllerParameterType.Float);
                    }
                }
            }
        }
        ...
        public static void generateAnimations()
        {
            ...
            foreach (HandSide side in new HandSide().Values())
            {
                foreach (FingerType finger in new FingerType().Values())
                {
                    foreach (FingerBendType joint in new FingerBendType().Values())
                    {
                        animationProcessed += generateSmoothingWeightAnimation(side, finger, joint, AnimationClipPosition.negative, PropertyType.smoothing_weight);
                        animationProcessed += generateSmoothingWeightAnimation(side, finger, joint, AnimationClipPosition.positive, PropertyType.smoothing_weight);
