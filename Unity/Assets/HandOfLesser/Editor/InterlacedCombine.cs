...
namespace HOL
{
    class InterlacedCombine
    {
        ...
        public static int generateFlipFlopBlendTree(BlendTree parent, List<ChildMotion> childTrees, HandSide side, FingerType finger, FingerBendType joint, PropertyType targetProperty)
        {
            ...
            tree.AddChild(generateValueDriverBlendtree(tree, side, finger, joint, PropertyType.input_interlaced, PropertyType.input), -0.2f);
            tree.AddChild(generateCombiningBlendTree(tree, side, finger, joint), -0.150f);
            tree.AddChild(generateCombiningBlendTree(tree, side, finger, joint), 0.150f);
            tree.AddChild(generateValueDriverBlendtree(tree, side, finger, joint, PropertyType.input_interlaced, PropertyType.input), 0.2f);
